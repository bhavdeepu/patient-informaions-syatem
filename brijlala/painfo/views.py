...
def patient_photo(request):
    ...
    qw=[]
    for a in daad:
        print(str(a.doc)[7:] + "=================================================asbdhkjasfbkajsdbkjasbdkjasdbkjasbfkjasbebqwkjdbqwdbqw")
    ...
    if request.method == 'POST':
        ...
        if formx.is_valid() and form2.is_valid():
            ...
            try:
                ...
                w = ipnos(ipno=ipno, uhid=name,name=last.name, date=date, post=last)
...
def paform(request):
 ...
 #                   qw = Painf(name=name, ipno=ipno, uhid=add_uhid, types=types, dep=dep,doc=a_file)
                ...
                q.save()
                w = ipnos(ipno=ipno,types=types,dep=dep,uhid=add_uhid,date=date,name=name,post=q)
                w.save()
                for a_file in fils:

                    qw=docs(doc=a_file,uhid=add_uhid,ipno=ipno,date=date,post=q,name=name)
                    qw.save()
